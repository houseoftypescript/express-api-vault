...
class VaultClient {
  ...
  public async setSecret<D>(path: string, data: D): Promise<boolean> {
    ...
    const response = await axios.post<{ destroyed: boolean }, AxiosResponse<{ destroyed: boolean }>, D>(
      url,
      data,
      configs
    );
...
export const vaultClient = new VaultClient({ token: VAULT_TOKEN, endpoint: VAULT_ENDPOINT });
